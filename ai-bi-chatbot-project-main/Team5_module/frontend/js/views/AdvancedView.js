...
function formatInsightScalar(value) {
...
function insightObjectToLine(item) {
  if (!item || typeof item !== "object" || Array.isArray(item)) {
    return "";
  }

  const directText = safeText(
    firstDefined(item?.insight, item?.message, item?.highlight, item?.text, item?.summary),
    ""
  );

  if (directText) {
    const metric = safeText(firstDefined(item?.metric, item?.type), "");
    return metric ? `${formatDataKeyLabel(metric)}: ${directText}` : directText;
  }

  const source = asObject(item?.data);
  const entries = Object.entries(Object.keys(source).length ? source : item);
  const scalarPairs = [];
  const skipKeys = new Set(["data", "details", "raw", "error", "traceback", "success", "type", "metric"]);

  for (const [key, rawValue] of entries) {
    if (skipKeys.has(String(key).toLowerCase())) {
      continue;
    }
    if (rawValue && typeof rawValue === "object") {
      continue;
    }
    const compact = formatInsightScalar(rawValue);
    if (!compact) {
      continue;
    }
    scalarPairs.push(`${formatDataKeyLabel(key)}: ${compact}`);
    if (scalarPairs.length >= 3) {
      break;
    }
  }
...
function collectInsightLines(value, maxLines = 12) {
  const lines = [];
  const seen = new Set();

  const pushLine = (rawText) => {
  ...
  const visit = (node, depth = 0) => {
    if (depth > 5 || lines.length >= maxLines || node === null || node === undefined) {
      return;
    }
    ...
    if (typeof node === "object") {
      const line = insightObjectToLine(node);
      if (line) {
        pushLine(line);
      }
...
const MODULE_DIAGNOSTIC_KEYS = new Set(["message", "error", "raw", "details", "traceback"]);
...
const ADVANCED_CACHE_STATE_KEY = "ui.advancedSummaryCache";
...
function isDiagnosticPayload(value) {
  ...
  return keys.every((key) => MODULE_DIAGNOSTIC_KEYS.has(key) || MODULE_IGNORED_KEYS.has(key));
}
...
export class AdvancedView {
  ...
  getAdvancedCacheMap() {
    return asObject(this.stateManager.get(ADVANCED_CACHE_STATE_KEY, {}));
  }

  saveAdvancedCache(fileId) {
    ...
    const nextCache = {
      ...this.getAdvancedCacheMap(),
      [safeFileId]: {
        advancedBundle: this.advancedBundle,
        advancedMeta: this.advancedMeta,
        activeAdvancedKey: this.activeAdvancedKey,
        cachedAt: new Date().toISOString()
      }
    };

    this.stateManager.updatePath(ADVANCED_CACHE_STATE_KEY, nextCache);
  }

  restoreAdvancedCache(fileId) {
    const safeFileId = safeText(fileId, "");
    if (!safeFileId) {
      return false;
    }

    const cacheEntry = asObject(this.getAdvancedCacheMap()?.[safeFileId]);
    const cachedBundle = asObject(cacheEntry?.advancedBundle);
    const cachedKeys = Object.keys(cachedBundle).filter((key) => !MODULE_IGNORED_KEYS.has(key));
    if (!cachedKeys.length) {
      return false;
    }

    this.advancedBundle = cachedBundle;
    this.advancedMeta = asObject(cacheEntry?.advancedMeta);

    const preferredKey = safeText(cacheEntry?.activeAdvancedKey, "");
    this.activeAdvancedKey = cachedKeys.includes(preferredKey) ? preferredKey : cachedKeys[0];

    this.renderAdvancedSection();
    return true;
  }

  clearAdvancedSectionState() {
    this.advancedBundle = {};
    this.advancedMeta = {
      successCount: 0,
      totalCount: 0,
  ...
  render() {
    return `
      <section class="dashboard-view dashboard-grid dashboard-grid--advanced" aria-label="Advanced Analytics">
        <div data-sidebar-mount class="sidebar-slot"></div>

        <article class="dashboard-main dashboard-main--advanced glass-card">
  ...
  syncState(state) {
    ...
    const nextFileId = state?.dataset?.fileId || null;
    const fileChanged = this.lastFileId !== nextFileId;
    this.fileId = nextFileId;
    this.lastFileId = nextFileId;

    if (fileChanged) {
      this.clearAdvancedSectionState();
      if (nextFileId) {
        this.restoreAdvancedCache(nextFileId);
      }
    }
  ...
  async tryAutoload() {
    ...
    if (analysisComplete) {
      this.restoreAdvancedCache(fileId);
  ...
  renderSummary(dashboard, team4, insightPayload = {}) {
    ...
    const insights = collectInsightLines(team4?.insights, 5);
    const nlpInsights = collectInsightLines(
      firstDefined(
        insightPayload?.insights,
        insightPayload?.highlights,
        insightPayload?.analysis?.highlights,
        insightPayload?.analysis?.insights
      ),
      5
    );
  ...
  renderAdvancedSection() {
    ...
    summaryEl.textContent = "Advanced modules loaded. Select a module to view analysis output.";

    const keys = Object.keys(this.advancedBundle).filter((key) => {
      return key !== "nlpStatus" && key !== "capabilities" && key !== "recentAnalyses";
    });
  ...
  renderAdvancedOutput() {
    ...
    if (!key) {
      outputEl.innerHTML = '<p class="table-empty">No advanced modules are available for this dataset.</p>';
      return;
    }
    ...
    const shouldShowNoData = !result.success || isDiagnosticPayload(payload) || !hasMeaningfulData(payload);
    ...
    if (shouldShowNoData) {
      outputEl.innerHTML = `
        ...
        <div class="glass-card" style="border: 1px solid var(--card-border); padding: 1rem; border-radius: 10px; background: var(--switch-bg); margin-top: 0.45rem;">
  ...
  formatValue(val) {
    ...
    if (typeof val === "boolean") {
      return val
        ? '<span style="color: var(--accent-primary); font-weight: 600;">Yes</span>'
    ...
    if (typeof val === "object") {
      ...
      if (!entries.length) return '<span style="color: var(--text-muted);">Empty</span>';
      return `<div style="background: var(--switch-bg); padding: 0.75rem; border-radius: 6px; border: 1px solid var(--switch-border); display: flex; flex-direction: column; gap: 0.5rem; width: 100%;">
        ${entries.map(([k, v]) => `
          <div style="display: flex; flex-direction: row; justify-content: space-between; border-bottom: 1px solid var(--switch-border); padding-bottom: 0.25rem;">
            <strong style="color: var(--text-primary); text-transform: capitalize; padding-right: 1rem; width: 40%; min-width: 120px;">${escapeHtml(formatDataKeyLabel(k))}</strong>
            <span style="width: 60%; text-align: left; word-break: break-word; color: var(--text-secondary);">${this.formatValue(v)}</span>
          </div>`).join("")}
      </div>`;
    }
  ...
  buildModulePreview(payload, moduleKey = "") {
    ...
    const normalizedModuleKey = safeText(moduleKey, "").toLowerCase().replace(/[\s_-]/g, "");

    if (normalizedModuleKey === "insights") {
      const insightCandidate = firstDefined(
        cleaned?.insights,
        cleaned?.highlights,
        cleaned?.analysis?.highlights,
        cleaned?.analysis?.insights,
        cleaned
      );
      const lines = collectInsightLines(insightCandidate, 20);
      if (lines.length) {
        return `
          <ul style="margin: 0; padding-left: 1.25rem; display: grid; gap: 0.5rem; list-style: disc; color: var(--text-secondary);">
            ${lines.map((line) => `<li>${escapeHtml(line)}</li>`).join("")}
          </ul>
        `;
      }
      return '<p class="table-empty">No insight highlights available for this dataset.</p>';
    }
    ...
    for (const [k, v] of Object.entries(cleaned)) {
      if (Array.isArray(v) && v.length > 0 && typeof v[0] === 'object') {
        ...
        return `
          <h5 style="color: var(--text-secondary); text-transform: uppercase; font-size: 0.85rem; letter-spacing: 0.05em; margin-bottom: 0.75rem;">
    ...
    const kvPairs = Object.entries(cleaned)
      .map(([k, v]) => {
        return `
          <div style="background: var(--switch-bg); border: 1px solid var(--switch-border); border-radius: 8px; padding: 1.25rem; display: flex; flex-direction: column; justify-content: flex-start; height: 100%;">
            <h5 style="color: var(--text-secondary); text-transform: uppercase; font-size: 0.85rem; letter-spacing: 0.05em; margin-bottom: 0.75rem;">
              ${escapeHtml(formatDataKeyLabel(k))}
            </h5>
  ...
  formatTableCellValue(value) {
    ...
    if (Array.isArray(value)) {
      return `<span style="color: var(--text-muted);">${value.length} item(s)</span>`;
    }

    if (typeof value === "object") {
      return `<span style="color: var(--text-muted);">${Object.keys(value).length} field(s)</span>`;
    }
    ...
    if (typeof value === "string") {
      ...
      if (looksStructured) {
        return '<span style="color: var(--text-muted);">Structured text omitted</span>';
      }
  ...
  selectAdvancedModule(key) {
    ...
    this.renderAdvancedSection();
    this.saveAdvancedCache(this.fileId);
  }
